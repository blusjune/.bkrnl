...
_this_prog=".bdx.0100.y.dmcache_setup.radiohead.sh";
_ver="20130123_192524";


#### radiohead info:
## 
## ## lsscsi
## root@radiohead $ lsscsi
## [0:0:0:0]    cd/dvd  TSSTcorp CDDVDW SH-S222A  SB01  /dev/sr0
## [2:0:0:0]    disk    ATA      ST2000DM001-1CH1 CC43  /dev/sda
## [3:0:0:0]    disk    ATA      ST2000DM001-1CH1 CC43  /dev/sdb
## [4:0:0:0]    disk    ATA      ST2000DM001-1CH1 CC43  /dev/sdc
## [5:0:0:0]    disk    ATA      ST2000DM001-1CH1 CC43  /dev/sdd
## [6:0:0:0]    disk    ATA      SAMSUNG MZ7PC128 CXM0  /dev/sde
## [7:0:0:0]    disk    ATA      SAMSUNG MZ7PC128 CXM0  /dev/sdf
## 
## ## mount
## root@radiohead $ mount | grep '/dev/'
## /dev/sda1 on / type ext4 (rw,errors=remount-ro)
## devpts on /dev/pts type devpts (rw,noexec,nosuid,gid=5,mode=0620)
## /dev/sdb2 on /var type ext4 (rw)
## /dev/sdc1 on /xd type ext4 (rw)
## /dev/sdd1 on /mnt/sdd1 type ext4 (rw)	## <- this should be unmounted before run-dmc (_srcdsk)
## /dev/sde1 on /mnt/sde1 type ext4 (rw)
## /dev/sdf1 on /mnt/sdf1 type ext4 (rw)	## <- this should be unmounted before run-dmc (_cache_media)
## 


echo "------------------------------------------------------------";
echo "$_this_prog ($_ver)";
echo "------------------------------------------------------------";
echo "IMPORTANT!!! before creating dm-cache, please make sure that
/dev/sdd (_srcdsk) and /dev/sdf (_cache_media) is NOT MOUNTED !!!";
echo "------------------------------------------------------------";
mount | grep '/dev/sd';
echo "------------------------------------------------------------";
read -p "#>> Are you sure? [y|n] " _ans;
if [ "X$_ans" != "Xy" ]; then
	echo "#>> come back again soon -- EXIT";
	exit 0;
fi
...
trial_04()	# it works! on machine 'prince'
{
umount /dev/sdc
umount /dev/sdb
...
## you can format it with mkfs.ext4 and mount it.
	...
	echo "";
	echo "#>> mount /dev/mapper/$_dmc_name /mnt/sdd1";
	mount /dev/mapper/$_dmc_name /mnt/sdd1;
